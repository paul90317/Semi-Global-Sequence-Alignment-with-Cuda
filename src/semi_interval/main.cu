...
#if END_MODE==0
__global__ void calculate(afg_unit* M,afg_unit* M1,afg_unit* M2,sequence x,sequence y,int offset_y)
#elif END_MODE==3
__global__ void calculate(afg_unit* M,afg_unit* M1,afg_unit* M2,sequence x,sequence y,int offset_y,res_unit_end* best_arr) 
#else
__global__ void calculate(afg_unit* M,afg_unit* M1,afg_unit* M2,sequence x,sequence y,int offset_y,res_unit_end* best_stack,int* bs_count,datatype* bscore) 
#endif
{
    int xid=TID;
...
int main(int argc,char** argv){
    ...
    sequence x(argv[1]);
    sequence y(argv[2]);
    std::cout<<"X sequence: "<<argv[1]<<" , Global interval=[1, "<<x.size()<<"]\n";
    std::cout<<"Y sequence: "<<argv[2]<<" , Global interval=[1, "<<y.size()<<"]\n";
    ...
    for(int offset_y=1;offset_y-x.size()<=y.size();offset_y++){
        ...
        #elif (END_MODE==3)
            calculate<<<nblock,nthread>>>(GM,GM1,GM2,x,y,offset_y,g_best_arr);
        #else
            calculate<<<nblock,nthread>>>(GM,GM1,GM2,x,y,offset_y,g_best_stack,g_bs_count,g_best_score);
        #endif
